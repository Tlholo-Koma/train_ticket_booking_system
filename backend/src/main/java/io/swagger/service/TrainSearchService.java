...
public class TrainSearchService {
    ...
    public List<Train> search(String source, String destination, Date travelDate) {
        ...
        for (Train train : trains) {
            if ((source == null || source.isEmpty() || train.getSourceStation().toLowerCase().contains(source.toLowerCase()))
                    && (destination == null || destination.isEmpty() || train.getDestinationStation().toLowerCase().contains(destination.toLowerCase()))
